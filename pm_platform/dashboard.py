...
XES_FILE   = UPLOADS_DIR / "uploaded_log.xes"
...
def api_upload():
    ...
    if not file.filename.endswith(".xes"):
        return jsonify({"error": "File must be an XES event log (.xes)"}), 400
        
    try:
        if XES_FILE.exists():
            XES_FILE.unlink()
        file.save(str(XES_FILE))
        
        # Auto-run pipeline
        py = python_exe()
        res_graph = run_script([py, "build_graph.py", str(XES_FILE)])
        res_analyze = run_script([py, "analyze_cycle_time.py", str(XES_FILE)])
        
        return jsonify({
            "message": "Upload & Analysis successful",
            "build_graph": res_graph,
            "analyze": res_analyze
        })
    except Exception as e:
...
<title>Generic Process Mining Platform</title>
...
<!-- SIDEBAR -->
<nav id="sidebar">
  <div class="sidebar-logo">🔍 Process Mining<span>Generic Analytics Platform</span></div>
  
  <div class="nav-label">Data</div>
  <div class="nav-item active" onclick="goPage('upload')" id="nav-upload">
    <span class="nav-icon">📤</span> Tải Lên (.xes)
  </div>

  <div class="nav-label">Analysis</div>
  <div class="nav-item" onclick="goPage('overview')" id="nav-overview">
...
<main id="main">
  ...
  <div class="page active" id="page-upload">
    <div class="page-title">📤 Tải Lên Event Log (.xes)</div>
    <div class="page-sub">Hệ thống chấp nhận tất cả các file định dạng chuẩn XES. Tự động chạy Pipeline sau khi tải.</div>

    <div class="card section-gap" style="text-align:center; padding: 60px 20px;">
      <h3 style="margin-bottom:20px; color:var(--text);">Chọn file XES để phân tích</h3>
      <input type="file" id="upload-file" accept=".xes" style="margin-bottom:20px; padding:10px; background:#090b12; border:1px dashed var(--border); border-radius:8px; width:400px; color:white;">
      <br>
      <button class="btn btn-primary" onclick="uploadXes()" id="btn-upload" style="padding:12px 30px; font-size:15px;">🚀 Tải lên & Phân tích tự động</button>
      <div id="upload-status" style="margin-top:20px; font-weight:600; color:var(--muted);"></div>
      <div id="upload-log" class="log-box" style="display:none; text-align:left; margin-top:20px; max-width:800px; margin-left:auto; margin-right:auto;"></div>
    </div>
  </div>
  ...
  <div class="page" id="page-overview">
    ...
    <div class="page-sub">Tổng quan quy trình (Process Overview) dựa trên file XES vừa tải</div>
...
async function uploadXes() {
  ...
  if (!fileInput.files.length) {
    alert("Vui lòng chọn một file .xes!");
    return;
  }
  const file = fileInput.files[0];
  const btn = document.getElementById('btn-upload');
  const statusElem = document.getElementById('upload-status');
  const logElem = document.getElementById('upload-log');

  btn.disabled = true;
  statusElem.innerHTML = "⏳ Đang tải lên và phân tích tự động... Quá trình này có thể mất 1-2 phút tuỳ dung lượng dữ liệu.";
  ...
  try {
    ...
    if (!res.ok) {
      statusElem.innerHTML = "❌ Lỗi tải lên: " + (data.error || "Unknown Error");
      btn.disabled = false;
      return;
    }
    
    // Log formatting
    let logHtml = `<div class="ok">✅ Tải lên thành công: ${file.name}</div>`;
    if (data.build_graph) {
      logHtml += `\n📦 Build Graph: ${data.build_graph.ok ? 'OK' : 'Lỗi'} \n${data.build_graph.stdout || data.build_graph.stderr}`;
    }
    if (data.analyze) {
      logHtml += `\n⏱ Analyze Cycle Time: ${data.analyze.ok ? 'OK' : 'Lỗi'} \n${data.analyze.stdout || data.analyze.stderr}`;
    }
    logElem.innerHTML = logHtml;
    logElem.style.display = 'block';
    statusElem.innerHTML = "✅ Phân tích hoàn tất! Dữ liệu mới đã sẵn sàng.";
    
    // Auto reload data in background
    await Promise.all([
      loadOverview(),
      loadCycleTime(),
      loadLoops()
    ...
    // Cập nhật lại Baseline cho Simulation
    await initSimPage();
    
    // Force re-init diagram variant list if needed
    const variantsList = document.getElementById('variants-list');
    if (variantsList) variantsList.innerHTML = '<div class="loading">Đang tải variants...</div>';
    ...
    statusElem.innerHTML = "❌ Lỗi mạng: " + e.message;
  }
  btn.disabled = false;
}
...
// ── SIMULATION (REDESIGNED DYNAMIC) ─────────────────────────────────────────
let SIM_ACTIVITIES = [];
let baselineTimes  = {};   // loaded from /api/summary
let scenarioHistory = [];  // [{name, timestamp, total_hours, total_days, steps, pct}]
let historyChart   = null;

async function initSimPage() {
  const data = await fetchJSON('/api/summary');
  if (!data || data.error || !data.per_activity_wait_hours) {
    document.getElementById('sim-form-sliders').innerHTML =
      '<div class="empty">Chưa có baseline. Hãy tải lên file XES và phân tích trước.</div>';
    return;
  }
  
  const per_act = data.per_activity_wait_hours;
  
  // Lấy Top 10 Activities có ảnh hưởng thời gian lớn nhất
  // (Tính bằng Median Time * Số lần xuất hiện)
  const actKeys = Object.keys(per_act)
    .filter(k => per_act[k].median_h !== undefined)
    .sort((a, b) => (per_act[b].median_h * per_act[b].count) - (per_act[a].median_h * per_act[a].count))
    .slice(0, 10);
  
  baselineTimes = {};
  SIM_ACTIVITIES = actKeys.map(k => {
    const val = parseFloat(per_act[k].median_h);
    baselineTimes[k] = val;
    return {
      key: k, 
      label: k.length > 25 ? k.substring(0, 22) + "..." : k,
      hint: `${per_act[k].count} lần`,
      max: Math.max(val * 4, 10)
    };
  });
